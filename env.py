...
class SumoEnvironment(MultiAgentEnv):
    ...
    def __init__(self, net_file, route_file, phases, out_csv_name=None, use_gui=True, num_seconds=20000, max_depart_delay=100000,
    ...
    def step(self, actions):
        ...
        print('self.traffic_signals1 :', self.traffic_signals)

        # run simulation for delta time
        for _ in range(self.yellow_time): 
            self._sumo_step()
        print ('self.ts_ids:', self.ts_ids)
        for ts in self.ts_ids:
            print('ts:', ts)
    ...
    def _compute_observations(self):
        ...
        for ts in self.ts_ids:
            ...
            print ('self.traffic_signals[ts].time_on_phase:', self.traffic_signals[ts].time_on_phase)
            print('self.max_green:', self.max_green)
            ...
            print('elapsed:{}, density:{}, queue:{}'.format(elapsed, density, queue))
    ...
    def _waiting_time_reward(self):
        ...
        for ts in self.ts_ids:
            ...
            print('ts_wait:', ts_wait)
            rewards[ts] = self.last_measure[ts] - ts_wait
            print('rewards[ts]:', rewards[ts])
            self.last_measure[ts] = ts_wait
            print('self.last_measure[ts]:', self.last_measure[ts])
